  ...
  useEffect(() => {
    ...
    const unsubscribePosts = onSnapshot(postsQuery, async (postsSnapshot) => {
        ...
        if (authorIds.length > 0) {
            ...
            await Promise.all(authorChunks.map(async chunk => {
                ...
                usersSnapshot.forEach(doc => {
                    const data = doc.data();
                    usersData[doc.id] = { id: doc.id, name: data.name, avatar: data.avatar, email: data.email, isVerified: data.isVerified };
                });
            }));
        }
        ...
        const fetchedPosts: Post[] = postsData.map(post => {
            ...
            return {
                ...post,
                user: { id: author.id, name: author.name || 'Unknown User', avatar: author.avatar || '', isVerified: author.isVerified },
            };
